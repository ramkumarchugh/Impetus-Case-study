...
export class ApplicationViewComponent implements OnInit, AfterViewInit {
  displayedColumns: string[] = ['sumAssured', 'noOfYears','baseplan', 'age', 'premium', 'salary', 'noOfDependents', 'status', 'workflow'];
  userid: number;
  applications: any = [];
  ...
  ngAfterViewInit() {
    ...
    this.restService.get<Application[]>('api/application/user/' + this.userid, null).subscribe(data => {
      ...
      this.dataSource = new MatTableDataSource(this.applications);
     // console.log(this.dataSource.data[55].baseplan.description);
